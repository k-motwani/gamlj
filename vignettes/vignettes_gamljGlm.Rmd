...
We can now probe the interaction. One can re-run the model adding the appropriate options to ask for simple effects or one can use the `simpleffects()` function, which is a convenience function to add simple effects to a pre-existing model. The function  `simpleffects()`, however, only returns the simple effects tables, not the full model. `simpleEffects()` accepts the same options as the `gamlj` function it applies on, with the addition of the formula (right hand sided) interface to declare the terms one wants to test.
...
#simpleEffects(mod3,simpleVariable = "xage",simpleModerator = "zexer")

simpleEffects(mod3,formula=~xage:zexer)
...
One can change the conditioning levels of the moderators with the `simpleScale` option (default is `mean_sd` for mean plus/minus one SD), either added to the `gamljGlm()` function or to the `simpleEffects()` function. If one wants to use the percentiles (25%,50%,75%), for instance, one can run the following.

```{r warning=FALSE, message=FALSE}
simpleEffects(mod3,formula=~xage:zexer ,simpleScale="percent")
```
